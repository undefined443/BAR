...
class CachedTokensFolder(datasets.DatasetFolder):
    """Dataset loader for pretokenized NPZ files.

    NPZ format:
        - 'tokens': Tokenized representation of original image
        - 'tokens_flip': Tokenized representation of horizontally flipped image
        - Randomly selects between original and flipped with 50% probability

    Args:
        root: Root directory path containing class subdirectories with NPZ files
        transform: Optional transform to apply (typically None for pretokenized data)
        target_transform: Optional transform to apply to target/label
    ...
    Reference: https://github.com/LTH14/mar/blob/main/util/loader.py
    """
    ...
    def _load_npz(path: str) -> np.ndarray:
        """Load NPZ file and randomly select between original and flipped tokens.

        Args:
            path: Path to NPZ file

        Returns:
            Token array (randomly selected between original and flipped)
        """
        data = np.load(path)
    ...
    def __getitem__(self, index: int) -> Tuple[int, torch.Tensor]:
        ...
        Returns:
            tuple: (target, tokens) where target is the class index
                   and tokens is the tokenized representation

        Note:
            Return order matches JSONL format for consistency: (class_id, tokens)
        """
        path, target = self.samples[index]
        tokens = self.loader(path)
