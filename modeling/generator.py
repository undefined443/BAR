...
class BAR(BaseModel):
    def __init__(self, config):
        ...
        if (
            ...
            raise ValueError(
                "latent grid dimensions must be divisible by patch_size."
            )
        ...
        rope = VisionRotaryEmbeddingFast(
            ...
            freqs_for='lang',
            num_cls_token=1 + self.repeat_class_condition,  # cls_token + repeated condition tokens
        )

        self.cls_token = nn.init.trunc_normal_(nn.Parameter(torch.zeros(1, 1, embed_dim)), 0., 0.02)
        ...
        self.pos_embed = nn.init.trunc_normal_(
            nn.Parameter(torch.zeros(1, self.image_seq_len + 128, embed_dim)), 0., 0.02)
        ...
        self.input_embeddings = nn.Embedding(target_codebook_size + 1, math.ceil(embed_dim / self.vq_split_channel))
        self.input_merge = nn.Linear(math.ceil(embed_dim / self.vq_split_channel) * self.vq_split_channel, embed_dim, bias=True)

        # Timestep embeddings for AdaLN conditioning
        self.timesteps_embeddings = nn.init.trunc_normal_(
            nn.Parameter(torch.zeros(1, self.image_seq_len + 100, embed_dim)), 0., 0.02)
    ...
    def sample_orders(self, x, random_ratio=None):
        ...
        raster_orders = torch.arange(self.image_seq_len, device=device).unsqueeze(0).expand(batch_size, -1)
        ...
        random_mask = (torch.rand(batch_size, 1, device=device) < random_ratio).expand(-1, self.image_seq_len)
    ...
    def get_raster_orders(self, x):
        ...
        raster_orders = torch.arange(self.image_seq_len, device=x.device).unsqueeze(0).expand(batch_size, -1)
    ...
    def forward_fn(self, input_ids, condition,
        ...
        if apply_generator_downsample and self.patch_size > 1:
            input_ids = input_ids.reshape(
                input_ids.shape[0], self.original_image_seq_len, self.base_vq_split_channel
            )
        ...
        repeated_condition_token = condition_token.unsqueeze(1).repeat(1, self.repeat_class_condition, 1)
        ...
        prefix = 1 + self.repeat_class_condition  # cls_token + repeated condition tokens
        ...
        target_orders = torch.cat([orders[:, 1:], orders[:, -1:]], dim=1)  # Shift by 1: [orders[1], ..., orders[-1], orders[-1]]
        ...
        first_target_embed = target_aware_pos_embed[
            torch.arange(batch_size, device=orders.device),
            orders[:, 0] + prefix
        ].unsqueeze(1).expand(-1, prefix, -1)  # [B, prefix, dim]
        ...
        if not is_sampling:
            ...
            embeddings = torch.cat([embeddings[:, :self.repeat_class_condition],
                                   self.shuffle(embeddings[:, self.repeat_class_condition:], orders)], dim=1)
        ...
        if orders is not None:
            ...
            rope_order_full = torch.cat([
                torch.arange(prefix, device=orders.device).unsqueeze(0).expand(batch_size, -1),  # [B, prefix]
                orders + prefix  # [B, image_seq_len] with positions [prefix, prefix+1, ...]
            ], dim=1)  # [B, seq_len]
            rope_order = rope_order_full[:, :seq_len]  # Slice to current sequence length
            ...
            prefix_targets = (orders[:, 0:1] + prefix).expand(-1, prefix)  # [B, prefix] all point to orders[0]
            # Shift orders by 1: [orders[1], orders[2], ..., orders[-1], orders[-1]]
            shuffled_targets = torch.cat([orders[:, 1:], orders[:, -1:]], dim=1) + prefix  # [B, image_seq_len]

            target_rope_order_full = torch.cat([prefix_targets, shuffled_targets], dim=1)  # [B, prefix + image_seq_len]
            target_rope_order = target_rope_order_full[:, :seq_len]  # Slice to current sequence length
        else:
            rope_order = None
        ...
        for idx, blk in enumerate(self.blocks):
            if self.use_checkpoint:
                x = torch.utils.checkpoint.checkpoint(
                        blk.forward, x, attn_mask, condition_token_with_time, rope_order, target_rope_order, use_reentrant=False)
            else:
                x = blk(x, attn_mask=attn_mask, c=condition_token_with_time, rope_order=rope_order, target_rope_order=target_rope_order)
    ...
    def generate(self,
        ...
        for step in range(self.image_seq_len):
            ...
            if guidance_scale <= 1.0:
                ...
                cfg_scale = 1. + (guidance_scale - 1.) * ratio
            ...
            if use_cfg:
                latent_conditions = self.forward_fn(
                    ...
                    orders=cfg_orders, is_sampling=True)[:, -1].reshape(-1, 1, self.lm_head.width)
            else:
                latent_conditions = self.forward_fn(
                    ids,
                    condition,
