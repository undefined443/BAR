...
class MaskBitModelingHead(nn.Module):
    ...
    def __init__(self, num_layers=3, width=2048, seq_len=16):
    ...
    def sample(self, conditions, guidance_scale=3.0, num_steps=50, use_cfg=False):
        ...
        x_t = torch.randn(
            batch_size, self.seq_len, device=device, dtype=conditions.dtype
        )

        # t from 1.0 (noisy) to 0.0 (clean), num_steps+1 points
        step_ts = torch.linspace(
            1.0, 0.0, num_steps + 1, device=device, dtype=conditions.dtype
        )
        ...
        for i in range(num_steps):
            ...
            eps_pred = (x_t - alpha_bar_cur.sqrt() * x0_pred) / (
                1 - alpha_bar_cur
            ).sqrt().clamp(min=1e-8)
            x_t = (
                alpha_bar_next.sqrt() * x0_pred + (1 - alpha_bar_next).sqrt() * eps_pred
            )
